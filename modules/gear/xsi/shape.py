...
def applyScaleShapeOp(shape, points):
    ...
    mesh = shape.Parent3DObject
    cls = mesh.ActivePrimitive.Geometry.AddCluster(c.siVertexCluster, "sn_ScaleShapeOpSour", points)

    # Apply Operator
    op = XSIFactory.CreateObject("sn_ScaleShapeOp")
...
def applySmoothShapeOp(shape, points):
    ...
    mesh = shape.Parent3DObject
    cls = mesh.ActivePrimitive.Geometry.AddCluster(c.siVertexCluster, "sn_SmoothShapeOp_Cls", points)

    # Apply Operator
    op = XSIFactory.CreateObject("sn_SmoothShapeOp")
