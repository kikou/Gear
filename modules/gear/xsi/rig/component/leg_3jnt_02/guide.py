## @package gear.xsi.rig.component.leg_2jnt_02.guide
...
class Guide(ComponentGuide):
    ...
    compType = "leg_3jnt_02" ## Component type
    compName = "leg" ## Component default name

    ## Description of the component
    description = "3 bones leg with stretch, roundess, ik/fk..."

    compatible = ["leg_3jnt_01"]

    # =====================================================
    ##
    # @param self
    def postInit(self):
        self.pick_transform = ["root", "knee", "ankle", "meta", "eff"]
        self.save_transform = ["root", "knee", "ankle", "meta", "eff"]
    ...
    def addObjects(self):
        ...
        self.meta = self.addLoc("meta", self.knee)
        self.eff = self.addLoc("eff", self.meta)

        centers = [self.root, self.knee, self.ankle, self.meta, self.eff]
    ...
    def addParameters(self):
        ...
        # Default Values
        self.pBlend       = self.addParam("blend", c.siInt4, 1, 0, 1)
        ...
        self.pDiv2 = self.addParam("div2", c.siInt4, 2, 1, None, 1, 10)
    ...
    def addLayout(self):
        ...
        item = group.addEnumControl(self.pRoll.scriptName, rollItems, "3rd Bone Roll with", c.siControlCombo)
...
#        item.setAttribute(c.siUINoLabel, True)
        ...
        row.addItem(self.pDiv2.scriptName, "Bone 2")
