...
def gear_GuideTools_Execute():
    ...
    dis.inspect(prop, 340, 295, 125, 200)
...
def gear_GuideToolsUI_Define(in_ctxt):
    ...
    prop.AddParameter3("templateInfos", c.siString, "", None, None, False, False)
...
def gear_GuideToolsUI_OnInit():
    ...
    item.SetAttribute(c.siUICY , 110)
    ...
    item = layout.AddString("componentInfos", "", True, 110)
    item.SetAttribute(c.siUINoLabel, True)
...
def gear_GuideToolsUI_extractCtrl_OnClicked():
    ...
    for rig_ctl in xsi.Selection:
        ...
        uti.setColor(guide_ctl, [.25,.35,.25])
